...
## v1.0.8 — Student Academic Promotions, Platform Usability Feedback, & Performance Optimization

Added student academic promotions, a platform usability feedback system, and structural/non-functional optimizations:
- **Automated Academic Promotion System**: Implemented automated, transaction-safe promotion of student levels under `/api/promotions`. Computes student CGPA: students with a CGPA $\ge$ 1.50 are promoted to the next level (or marked as `GRADUATED` if they reached their department's `maxLevel`), while students below 1.50 repeat their current level. Records detailed records in the `PromotionHistory` table.
- **Usability Feedback System**: Implemented a platform-wide feedback collection system under `/api/feedback`. Displays a non-intrusive survey modal asking students and lecturers to evaluate the platform from 1 to 5 stars across five core TAM metrics (Ease of Use, Interface Design, Reliability, Functionality, Performance) with optional comments.
- **Admin Feedback Dashboard & CSV Export**: Added an administrative panel at `/admin/feedback` displaying real-time feedback scores, overall response counts, and average star ratings. Administrators can download the raw submission data in standard CSV format for offline analysis.
- **Database Connection Retry Client**: Extended the Prisma client in [prisma.ts](file:///home/themw/DEV/Project_DEV/OPENCODE/Portal-V1/eduportal/apps/api/src/lib/prisma.ts) using query hook extensions to catch transient PostgreSQL/Neon connection drops and pooler timeouts (e.g., error codes `P2024`, `P2028`, `P2010`), automatically retrying failed queries up to three times with exponential backoff.
- **Next.js Image Component Migration**: Replaced legacy `<img>` tags with the Next.js `<Image>` component to serve optimized modern WebP formats and generate device-appropriate responsive sizes. Critical above-the-fold assets, such as branding logos in [logo.tsx](file:///home/themw/DEV/Project_DEV/OPENCODE/Portal-V1/eduportal/apps/web/src/components/ui/logo.tsx), use the `priority` prop to optimize Largest Contentful Paint (LCP) times.
- **Hydration and Navigation Guard**: Updated [use-auth-guard.ts](file:///home/themw/DEV/Project_DEV/OPENCODE/Portal-V1/eduportal/apps/web/src/hooks/use-auth-guard.ts) to postpone authentication checks and redirect routing until the component is mounted and the Zustand persistence store is hydrated. This prevents unexpected redirects to `/login` when users reload nested routes (such as `/admin/promotions`).
- **Dynamic Lazy Loading**: Optimized LCP by dynamically importing the new `FeedbackSurveyModal` client-side, reducing JavaScript blocking execution on initial page load.
